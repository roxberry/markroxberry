...
add_action( 'widgets_init', 'widephoto_widgets_init' );
...
function widephoto_enqueue_scripts () {
    ...
    wp_register_script('supersized', get_template_directory_uri().'/supersized/js/supersized.3.1.3.js', 'jquery', '3.1.3', false );
    wp_register_script('supersizedImgs', get_template_directory_uri().'/supersized/js/supersized.imgs.js', 'supersized', '1.0', false );
	
    
    //enqueue scripts
    wp_enqueue_script( 'supersized' );
    wp_enqueue_script( 'supersizedImgs' );
    
}
add_action('wp_enqueue_scripts', 'widephoto_enqueue_scripts');

/* 
* Widephoto enqueue styles
*/
function widephoto_enqueue_styles() {
    
        wp_register_style( 'supersized', get_template_directory_uri().'/supersized/css/supersized.css', 'supersized' );
        wp_enqueue_style( 'supersized' );
   
}
add_action( 'wp_enqueue_scripts', 'widephoto_enqueue_styles' );
...
function widephoto_add_supersized_script () { 
		...
		$widephoto_option = widephoto_get_global_options();
		$slides = array('wdp_lite_bg_image_1','wdp_lite_bg_image_2','wdp_lite_bg_image_3');
		$i=1;
		$j=0;
        $supersizedVars = array();
		
		//Count background images set by the user
		foreach ($slides as $slide) {
			if($widephoto_option[$slide]<>'') $j++;
		}			
		
		//If there are any images print them out
		if ($j != 0) {
            
			 foreach ($slides as $slide) {
				
				if ($widephoto_option[$slide]<>'') {
				
					 $supersizedVars[$i] = $widephoto_option[$slide]; 
				}
		...
		// Else echo default images
		else { 
            $supersizedVars[1] = get_template_directory_uri().'/images/before-the-storm.jpg';
            $supersizedVars[2] = get_template_directory_uri().'/images/fisherman.jpg';
            $supersizedVars[3] = get_template_directory_uri().'/images/castle.jpg';
       }
...
add_action('wp_enqueue_scripts', 'widephoto_add_supersized_script');	
...
if(is_admin()){	
	require_once('lib/pl-theme-settings-basic.php');
}
 ...
 * @return array
 */
function widephoto_get_global_options(){
	
	$widephoto_option = array();

	$widephoto_option = get_option('widephoto');
	
	if ($widephoto_option === false)
	{
		$widephoto_option['wdp_lite_bg_image_1'] = '';
		$widephoto_option['wdp_lite_bg_image_2'] = '';
		$widephoto_option['wdp_lite_bg_image_3'] = '';
	}
	
return $widephoto_option;
}
 ...
 * <?php echo $widephoto_option['widephoto_txt_input']; ?>
 *
 * Note: Should you notice that the variable ($widephoto_option) is empty when used in certain templates such as header.php, sidebar.php and footer.php
...
/**
* Wrap the videos around a container in order to apply styles
*
*/ 

add_filter( 'embed_oembed_html', 'widephoto_oembed_filter', 10, 4 ) ;
 
function widephoto_oembed_filter($html, $url, $attr, $post_ID) {
    $return = '<figure class="video-container">'.$html.'</figure>';
    return $return;
}

// Remove Gallery Width and Height HTML
