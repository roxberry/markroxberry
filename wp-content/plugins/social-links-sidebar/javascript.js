...
function social_links_sidebar_ajax_saveOrder(){
...
function addLoadingIcon(){
	var img = Builder.node('span',{
						...
						Builder.node('img',{src:'/wp-content/plugins/about-me/images/ajax-loader.gif',style:'margin:2px'},''));
	//console.log(img);
...
function social_links_ajax_addNetwork(selectedIndex,textInput,responseDiv){
	...
	mysack.execute = 1;
	mysack.method = 'POST';
	mysack.setVar( "action", "social_links_sidebar_add_network" );
...
function social_links_ajax_delete_network(linkId){
	...
	mysack.execute = 1;
	mysack.method = 'POST';
	mysack.setVar( "action", "social_links_sidebar_delete_network" );
